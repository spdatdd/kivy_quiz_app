...
class QuizApp(MDApp):
    ...
    # danh sách bài thi hiện tại hiển thị
    eternal_data = ListProperty([])
    local_data = ListProperty([])
    # danh sách
    eternal_list = local_list = temp_local_list = did_exam_list = logger = []
    # trạng thái load danh sách
    eternal_refreshing = local_refreshing = BooleanProperty()
    # trạng thái load bài thi 
    ...
    def set_theme(self, type):
        self.theme_cls.theme_style = type
    ...
    def go_home (self):
        ...
        if response != 'internet not available':
            # online 
    ...
    def check_network_available(self):
        ...
        Kiểm tra kết nối mạng
            None
        Returns:
            self.mode = 'online': nếu khả dụng
            self.mode = 'offline': ngược lại
        """
        try:
    ...
    def make_request (self, url, data={}, type_result='text', method='get'):
        ...
        if self.is_network_available():
            try:
                ...
                self.tolog(f"HTTP Error: {errh}")
            except requests.exceptions.ConnectionError as errc:
                self.tolog(f"Error Connecting: {errc}")
            except requests.exceptions.Timeout as errt:
                self.tolog(f"Timeout Error: {errt}")
            except requests.exceptions.RequestException as err:
                self.tolog(f"Request Exception: {err}")
            finally:
                return result
        else:
    ...
    def danh_sach_bai_thi_home(self):
        ...
        if len(self.eternal_list) == 0:
            self.make_request(url="https://tieu0luan0tot0nghiep.000webhostapp.com/get_external_exam.php",
                              data={}, 
                              type_result='json',
                              method='get')
